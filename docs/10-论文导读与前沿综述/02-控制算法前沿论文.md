...
| 属性 | 内容 |
|------|------|
| **标题** | From PID to Active Disturbance Rejection Control |
| **作者** | Han, J. (韩京清) |
| **年份** | 2009（综述），1998（原始提出） |
| **期刊** | IEEE Transactions on Industrial Electronics |
| **引用次数** | 5000+（ADRC 相关论文合计） |
| **推荐等级** | ★★★ 必读 |
| **研究方向** | 自抗扰控制 (ADRC) |

> 📄 DOI: [From PID to Active Disturbance Rejection Control](https://doi.org/10.1109/TIE.2009.2011351)

### 核心思想

ADRC 将系统内部不确定性和外部扰动统一视为 **"总扰动"**，通过扩张状态观测器（ESO）实时估计并补偿，实现对未知扰动的主动抑制。

### ADRC 三大核心组件

**1. 跟踪微分器（Tracking Differentiator, TD）：**

$$\dot{r}_1 = r_2$$
$$\dot{r}_2 = -R \, \text{sat}(r_1 - v, \, r_2 / R)$$

其中 $v$ 是参考输入，$R$ 是速度因子，$\text{sat}$ 是饱和函数。
...
其中 $\hat{z}_1$ 估计输出，$\hat{z}_2$ 估计输出导数，$\hat{z}_3$ 估计总扰动（包含模型不确定性和外扰），$b_0$ 是控制增益估计值。

**3. 非线性状态误差反馈（NLSEF）：**

$$u_0 = k_1 \, \text{fal}(e_1, \alpha_1, \delta) + k_2 \, \text{fal}(e_2, \alpha_2, \delta)$$
$$u = \frac{u_0 - \hat{z}_3}{b_0}$$

其中 $e_1 = r_1 - \hat{z}_1$，$e_2 = r_2 - \hat{z}_2$，$\text{fal}(\cdot)$ 是非线性函数：

$$\text{fal}(e, \alpha, \delta) = \begin{cases} |e|^\alpha \text{sign}(e), & |e| > \delta \\ e / \delta^{1-\alpha}, & |e| \leq \delta \end{cases}$$

### ADRC vs PID vs MPC 对比

| 对比维度 | PID | ADRC | MPC |
|---------|-----|------|-----|
| 模型依赖 | 不依赖 | 不依赖（估计扰动） | 依赖精确模型 |
| 抗扰能力 | 被动（误差驱动） | 主动（估计+补偿） | 前馈+反馈 |
| 参数数量 | 3 个 | 6~10 个 | Q, R, N 等 |
| 计算量 | 极低 | 低 | 高 |
| 非线性处理 | 差 | 好 | 好（NMPC） |
| 工程实现 | 简单 | 中等 | 复杂 |
