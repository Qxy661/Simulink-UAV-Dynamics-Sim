...
> 📄 DOI: [AirSim: High-Fidelity Visual and Physical Simulation for Autonomous Vehicles](https://doi.org/10.1007/978-3-030-05114-3_5)
...
仿真保真度（Simulation Fidelity）是衡量仿真平台可信度的核心指标。以下从四个维度定义关键保真度指标：

### 8.1 物理保真度

| 指标 | 定义 | 评估方法 |
|------|------|---------|
| 动力学误差 | 仿真状态与真实状态的偏差 | $\epsilon_{dyn} = \frac{1}{T}\int_0^T \|\mathbf{x}_{sim}(t) - \mathbf{x}_{real}(t)\| dt$ |
| 气动模型精度 | 推力/力矩与真实值的偏差 | 风洞实验对比 |
| 碰撞响应真实性 | 碰撞后运动学/动力学合理性 | 物理实验对比 |
| 实时因子（RTF） | 仿真时间与墙钟时间之比 | $\text{RTF} = \frac{t_{sim}}{t_{wall}}$，理想值为 1.0 |

### 8.2 传感器保真度

| 指标 | 定义 | 典型要求 |
|------|------|---------|
| 噪声模型准确性 | 仿真噪声与真实噪声分布匹配度 | Allan 方差对比 |
| 时间戳精度 | 传感器数据时间戳与仿真时钟的同步误差 | < 1 ms |
| 视场角/分辨率 | 相机/LiDAR 参数与真实硬件一致 | 参数可配 |
| 偏置漂移建模 | IMU 偏置随时间的漂移特性 | 随机游走模型 |

### 8.3 视觉保真度

| 指标 | 定义 | 评估方法 |
|------|------|---------|
| 渲染分辨率 | 图像像素分辨率 | 与真实相机对比 |
| 光照模型 | 全局光照、阴影、反射的真实性 | 主观评分 / FID 指标 |
| 纹理质量 | 场景纹理的细节程度 | 分辨率 / MIP 等级 |
| 帧率 | 渲染帧率（FPS） | 实时要求 > 30 FPS |

### 8.4 时序保真度

| 指标 | 定义 | 典型要求 |
|------|------|---------|
| 仿真步长 | 物理仿真最小时间步 | < 1 ms（飞控相关） |
| 通信延迟 | 平台间数据传输延迟 | < 10 ms（HIL） |
| 同步精度 | 多平台时钟同步误差 | < 1 ms（锁步仿真） |
| 抖动（Jitter） | 周期性事件的时间偏差 | < 10% 步长 |

### 8.5 补充：NVIDIA Isaac Sim

> 📄 参考：[NVIDIA Isaac Sim 官方文档](https://docs.omniverse.nvidia.com/isaacsim/latest/overview.html)

NVIDIA Isaac Sim 是基于 Omniverse 平台的高保真机器人仿真环境，可视为 AirSim 的工业级替代方案：

| 对比维度 | AirSim (已停) | Isaac Sim |
|---------|--------------|-----------|
| 渲染引擎 | Unreal Engine 4 | Omniverse (RTX) |
| 光线追踪 | 不支持 | 原生 RTX 光追 |
| 物理引擎 | PhysX / FastPhysics | PhysX 5 / Flex |
| GPU 加速 | 有限 | 深度 GPU 加速 |
| ROS 集成 | ROS 1 Bridge | ROS 2 原生 |
| 域随机化 | 手动实现 | 内置支持 |
| 许可证 | 开源（已停） | 免费（非商业） |
